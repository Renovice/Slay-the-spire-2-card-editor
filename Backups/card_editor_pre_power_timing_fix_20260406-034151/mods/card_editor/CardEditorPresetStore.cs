...
internal static class CardEditorPresetStore
{
		...
		public CardUpgradeOverride ToUpgradeSafe()
		{
			...
			if (ExtraEffects != null && ExtraEffects.Count > 0)
			{
				...
				foreach (CardExtraEffectDto? dto in ExtraEffects)
				{
					...
					if (!dto.TryToEffect(out CardExtraEffect effect))
					{
						parsed.Add(null!);
						continue;
					}
	...
	internal sealed class CardExtraEffectDto
	{
		...
		public bool TryToEffect(out CardExtraEffect effect)
		{
			...
			effect.TriggerEveryN = Math.Max(0, TriggerEveryN);
			effect.TriggerMaxFires = Math.Max(0, TriggerMaxFires);
			effect.TriggerMaxTurns = Math.Max(0, TriggerMaxTurns);
			...
			effect.RepeatCount = RepeatCount <= 0 ? 1 : RepeatCount;
