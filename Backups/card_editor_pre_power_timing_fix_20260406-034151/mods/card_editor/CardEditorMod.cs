...
internal static class TemporaryStatDurationDescriptionFix
{
	...
	public static void TryFix(CardModel card, ref string description)
	{
		...
		if (!TryGetOverrideData(card.Id, out CardOverride? overrideData))
		{
			return;
		}
	...
	private static bool TryGetOverrideData(ModelId cardId, out CardOverride? overrideData)
	{
		overrideData = null;

		if (CardEditorUiState.TryGetDraftOverride(cardId, out CardOverride draft) && HasAnyDurationOverrides(draft))
		{
			overrideData = draft;
			return true;
		}

		if (CardEditorOverrides.TryGet(cardId, out CardOverride stored) && HasAnyDurationOverrides(stored))
		{
			overrideData = stored;
			return true;
		}
...
internal static class RetainHandDurationDescriptionFix
{
	...
	public static void TryFix(CardModel card, ref string description)
	{
		...
		int? overrideTurns = TryGetTurnsFromOverride(card.Id);
	...
	private static int? TryGetTurnsFromOverride(ModelId cardId)
	{
		if (CardEditorUiState.TryGetDraftOverride(cardId, out CardOverride draft)
			&& draft.PowerAmounts != null
			&& draft.PowerAmounts.TryGetValue(_retainHandPowerId, out decimal draftAmount))
		{
			return (int)draftAmount;
		}
...
internal static class NoDrawDurationDescriptionFix
{
	...
	private static int? TryGetTurnsFromOverride(ModelId cardId)
	{
		if (CardEditorUiState.TryGetDraftOverride(cardId, out CardOverride draft)
			&& draft.PowerAmounts != null
			&& draft.PowerAmounts.TryGetValue(_noDrawPowerId, out decimal draftAmount))
		{
			return (int)draftAmount;
		}
...
internal static class ConquerorDurationDescriptionFix
{
	...
	private static int? TryGetTurnsFromOverride(ModelId cardId)
	{
		if (CardEditorUiState.TryGetDraftOverride(cardId, out CardOverride draft)
			&& draft.PowerAmounts != null
			&& draft.PowerAmounts.TryGetValue(_conquerorPowerId, out decimal draftAmount))
		{
			return (int)draftAmount;
		}
...
internal static class ReflectDurationDescriptionFix
{
	...
	private static int? TryGetTurnsFromOverride(ModelId cardId)
	{
		if (CardEditorUiState.TryGetDraftOverride(cardId, out CardOverride draft)
			&& draft.PowerAmounts != null
			&& draft.PowerAmounts.TryGetValue(_reflectPowerId, out decimal draftAmount))
		{
			return (int)draftAmount;
		}
...
internal static class ColossusDurationDescriptionFix
{
	...
	private static int? TryGetTurns(CardModel card)
	{
		...
		{
			if (draft.PowerAmounts != null && draft.PowerAmounts.TryGetValue(_colossusPowerId, out decimal draftAmount))
			{
				return (int)draftAmount;
			}
			if (draft.DynamicVarBaseValues != null && draft.DynamicVarBaseValues.TryGetValue("Colossus", out decimal draftTurns))
			{
				return (int)draftTurns;
			}
		}
...
internal static class BlurDurationDescriptionFix
{
	...
	private static int? TryGetTurns(CardModel card)
	{
		...
		if (CardEditorUiState.TryGetDraftOverride(card.Id, out CardOverride draft))
		{
			if (draft.PowerAmounts != null && draft.PowerAmounts.TryGetValue(_blurPowerId, out decimal draftAmount))
			{
				return (int)draftAmount;
			}
			if (draft.DynamicVarBaseValues != null && draft.DynamicVarBaseValues.TryGetValue("Blur", out decimal draftTurns))
			{
				return (int)draftTurns;
			}
		}
...
internal static class HandTrickSlyDurationDescriptionFix
{
	...
	public static void TryFix(CardModel card, ref string description)
	{
		...
		CardOverride? overrideData = null;
		if (CardEditorUiState.TryGetDraftOverride(card.Id, out CardOverride draft) && draft.SlyGrantDuration != null)
		{
			overrideData = draft;
		}
		else if (CardEditorOverrides.TryGet(card.Id, out CardOverride stored) && stored.SlyGrantDuration != null)
		{
			overrideData = stored;
		}
...
internal static class SealedThroneDescriptionFix
{
	...
	public static void TryFix(CardModel card, ref string description)
	{
		...
		int starsGained = TryGetStarsGained(card.Id) ?? 1;
	...
	private static int? TryGetStarsGained(ModelId cardId)
	{
		if (CardEditorUiState.TryGetDraftOverride(cardId, out CardOverride draft)
			&& draft.PowerAmounts != null
			&& draft.PowerAmounts.TryGetValue(_sealedThronePowerId, out decimal draftAmount))
		{
			return (int)draftAmount;
		}
...
internal static class KinglyKickDescriptionFix
{
	...
	public static void TryFix(CardModel card, ref string description)
	{
		...
		int? reduction = TryGetReduction(card.Id);
	...
	private static int? TryGetReduction(ModelId cardId)
	{
		if (CardEditorUiState.TryGetDraftOverride(cardId, out CardOverride draft) && draft.DrawCostReduction.HasValue)
		{
			return draft.DrawCostReduction.Value;
		}

		if (CardEditorOverrides.TryGet(cardId, out CardOverride stored) && stored.DrawCostReduction.HasValue)
		{
...
internal static class ResonanceEnemyStrengthLossDescriptionFix
{
	...
	public static void TryFix(CardModel card, ref string description)
	{
		...
		if (!TryGetLoss(card.Id, out int loss))
		{
			return;
		}
	...
	private static bool TryGetLoss(ModelId cardId, out int loss)
	{
		loss = 1;

		if (CardEditorUiState.TryGetDraftOverride(cardId, out CardOverride draft))
		{
			if (draft.DynamicVarBaseValues != null
				&& draft.DynamicVarBaseValues.TryGetValue(CardEditorOverrideKeys.ResonanceEnemyStrengthLoss, out decimal draftLoss))
			{
				loss = (int)draftLoss;
			}
			return true;
...
internal static class HardcodedPowerAmountDescriptionFix
{
	...
	public static void TryFix(CardModel card, ref string description)
	{
		...
		if (!TryGetOverrideData(card.Id, out CardOverride? overrideData) || overrideData.PowerAmounts == null || overrideData.PowerAmounts.Count == 0)
		{
			return;
		}
...
public static class KinglyKick_AfterCardDrawn_CostReductionOverride_Patch
{
	public static void Postfix(KinglyKick __instance, CardModel card)
	{
		...
		if (!CardEditorOverrides.TryGet(__instance.Id, out CardOverride overrideData) || !overrideData.DrawCostReduction.HasValue)
		{
			return;
		}
...
public static class CardLibrary_ShowCardDetail_Patch
{
	public static bool Prefix(NCardHolder holder, NCardLibrary __instance)
	{
		...
		NCardEditorPopup popup = NCardEditorPopup.Create(preview, () => CardEditorUiState.RefreshLibrary(__instance), useModalContainer: false);
		Callable.From(() =>
		{
			Log.Info($"[CardEditor] Adding popup to NGame (instance={NGame.Instance != null})");
...
public static class PowerCmd_Apply_Patch
{
	...
	public static void Prefix(PowerModel power, ref decimal amount, CardModel? cardSource)
	{
		...
		if (!CardEditorOverrides.TryGet(durationSourceCard.Id, out CardOverride overrideData))
		{
			return;
		}
