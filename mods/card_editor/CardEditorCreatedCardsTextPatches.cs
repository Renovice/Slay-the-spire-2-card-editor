...
internal static class CreatedCardTextBuilder
{
	...
	public static string Build(CardModel card, Creature? target, bool isUpgradePreview)
	{
		...
		// Preview-only custom upgrade text: shown ONLY on the upgrade screen, bypassing the auto green-diff
		// so the user fully controls the green markup (their text is final). Never appears on the played card.
		if (isUpgradePreview)
		{
			string? previewOnly = CardEditorCreatedCardsStore.GetCustomUpgradePreviewText(card.Id);
			if (previewOnly != null)
			{
				string previewAuto = BuildAutoGeneratedDescription(card, target, isUpgradePreview);
				IReadOnlyList<string> previewFallback = CardEditorExtraEffects.GetLiveNumberFallbackTokensForDescription(card, isUpgradePreview);
				IReadOnlyDictionary<string, string> previewStable = CardEditorExtraEffects.GetStableLiveNumberTokensForDescription(card, isUpgradePreview);
				string previewSynced = CardEditorDescriptionNumberHighlighter.ApplyLiveNumbersAndManagedLinesFromReference(
					previewOnly,
					previewAuto,
					previewFallback,
					previewStable);
				string previewRendered = CardEditorVanillaKeywordSupport.FormatDescription(card, previewSynced, target, isUpgradePreview);
				return CardEditorDescriptionNumberHighlighter.ResolvePreviewOnlyHighlightMarkers(previewRendered, isUpgradePreview);
			}
		}
		...
		if (wantsUpgradedText)
		{
			...
			if (customUpgraded != null)
			{
				...
				string finalUpgraded = isUpgradePreview ? ApplyUpgradePreviewNumericHighlight(card, target, rendered, baseCustom) : rendered;
				return CardEditorDescriptionNumberHighlighter.ResolvePreviewOnlyHighlightMarkers(finalUpgraded, isUpgradePreview);
			}
		}
		...
		if (customText != null)
		{
			...
			string syncedText = CardEditorDescriptionNumberHighlighter.ApplyLiveNumbersAndManagedLinesFromReference(
				...
				stableLiveNumbers);
			string rendered = CardEditorVanillaKeywordSupport.FormatDescription(card, syncedText, target, isUpgradePreview);
			string finalBase = isUpgradePreview ? ApplyUpgradePreviewNumericHighlight(card, target, rendered, customText) : rendered;
			return CardEditorDescriptionNumberHighlighter.ResolvePreviewOnlyHighlightMarkers(finalBase, isUpgradePreview);
		}
