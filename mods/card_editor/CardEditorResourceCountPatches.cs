	...
	private static async Task TrackAfter(Task original, CombatState combatState, Creature creature, decimal amount)
	{
		...
		try
		{
			...
			if (delta > 0)
			{
				...
				await CardEditorExtraEffects.TriggerPowerCountEventAsync(combatState, creature, CardExtraEffectCountEvent.BlockGained);
			}
		}
		catch (Exception ex)
		{
...
internal static class Hook_AfterEnergySpent_CardEditorPowerCountEvent_Patch
{
	...
	private static async Task TrackAfter(Task original, CombatState combatState, Creature creature, int amount)
	{
		...
		try
		{
			...
			if (delta > 0)
			{
				await CardEditorExtraEffects.TriggerPowerCountEventAsync(combatState, creature, CardExtraEffectCountEvent.EnergyUsed);
			}
		}
		catch (Exception ex)
		{
...
internal static class Hook_AfterDamageGiven_CardEditorPowerCountEvent_Patch
{
	...
	private static async Task TrackAfter(Task original, CombatState combatState, Creature dealer, DamageResult results)
	{
		...
		try
		{
			...
			if (delta > 0)
			{
				await CardEditorExtraEffects.TriggerPowerCountEventAsync(combatState, dealer, CardExtraEffectCountEvent.DamageDealt);
			}
		}
		catch (Exception ex)
		{
...
internal static class Hook_AfterDamageReceived_CardEditorPowerCountEvent_Patch
{
	...
	private static async Task TrackAfter(Task original, CombatState combatState, Creature target, DamageResult result)
	{
		...
		try
		{
			...
			if (delta > 0)
			{
				await CardEditorExtraEffects.TriggerPowerCountEventAsync(combatState, target, CardExtraEffectCountEvent.DamageTaken);
			}
		}
		catch (Exception ex)
		{
...
internal static class Hook_AfterSummon_CardEditorPowerCountEvent_Patch
{
	...
	private static async Task TrackAfter(Task original, CombatState combatState, Creature creature, decimal amount)
	{
		...
		try
		{
			...
			if (delta > 0)
			{
				await CardEditorExtraEffects.TriggerPowerCountEventAsync(combatState, creature, CardExtraEffectCountEvent.Summoned);
			}
		}
		catch (Exception ex)
		{
...
internal static class PlayerCmd_GainEnergy_CardEditorResourceCount_Patch
{
	...
	private static async Task TrackAfter(Task original, Player player, int oldEnergy, CardExtraEffectCountEvent countEvent, bool gain)
	{
		...
		try
		{
			...
			CombatState? state = actor.CombatState;
			...
			if (delta > 0 && state != null)
			{
				...
				await CardEditorExtraEffects.TriggerPowerCountEventAsync(state, actor, countEvent);
			}
		}
		catch (Exception ex)
		{
	...
	public static void Postfix(Creature __instance, int __state)
	{
		...
		try
		{
			...
			if (delta > 0 && __instance.CombatState != null)
			{
				...
				CardEditorExtraEffects.TriggerPowerCountEvent(__instance.CombatState, __instance, CardExtraEffectCountEvent.BlockLost);
			}
		}
		catch (Exception ex)
		{
...
internal static class Creature_AfterTurnStart_CardEditorResourceCount_Patch
{
	...
	{
		if (__result == null || __instance?.CombatState == null)
		{
			return;
		}
	...
	private static async Task TrackAfter(Task original, Creature creature, int oldBlock)
	{
		...
		try
		{
			...
			if (delta > 0 && creature.Block <= 0)
			{
				...
				CardEditorExtraEffects.TriggerPowerCountEvent(combatState, creature, CardExtraEffectCountEvent.BlockLost, CardEditorExtraEffects.ResourceCountSource.BetweenTurnsBlockClear);
			}
		}
		catch (Exception ex)
		{
	...
	private static async Task TrackAfter(Task original, Creature creature, int oldHp)
	{
		...
		try
		{
			...
			if (delta > 0)
			{
				...
				await CardEditorExtraEffects.TriggerPowerCountEventAsync(combatState, creature, CardExtraEffectCountEvent.HealingReceived);
			}
		}
		catch (Exception ex)
		{
...
internal static class Hook_AfterPowerAmountChanged_CardEditorPowerCountEvent_Patch
{
	...
	private static async Task TrackAfter(Task original, CombatState combatState, PowerModel power, decimal amount)
	{
		...
		try
		{
			...
			if (amount > 0m)
			{
				await CardEditorExtraEffects.TriggerPowerCountEventAsync(combatState, owner, CardExtraEffectCountEvent.StatusGained, triggeringPower: power);
			}
			else if (amount < 0m)
			{
				await CardEditorExtraEffects.TriggerPowerCountEventAsync(combatState, owner, CardExtraEffectCountEvent.StatusLost, triggeringPower: power);
			}
		}
		catch (Exception ex)
		{
