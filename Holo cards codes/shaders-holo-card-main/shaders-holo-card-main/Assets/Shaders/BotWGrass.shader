// This shader is modified from my original BotW grass shader, based on work by Roystan.
Shader "Custom/BotWGrass"
{
	Properties
	{
		...
		_BladePosDelta("Blade Position Offset", Range(0, 0.2)) = 0.05
	...
	SubShader
	{
		...
		HLSLINCLUDE
			...
			GeomOutput TransformGeomToClip(float3 pos, float3 offset, float3x3 transformationMatrix, float2 uv)
			{
				GeomOutput o;

				o.pos = TransformObjectToHClip(pos + mul(transformationMatrix, offset));
				o.uv = uv;
				o.worldPos = mul(unity_ObjectToWorld, pos + mul(transformationMatrix, offset));
			...
			void geom(point VertexOutput input[1], inout TriangleStream<GeomOutput> triStream)
			{
				float3 positionOS = input[0].vertex.xyz;

				positionOS.x += (rand(positionOS.xzx) - 0.5f) * _BladePosDelta;
				positionOS.z += (rand(positionOS.zxx) - 0.5f) * _BladePosDelta;
				...
				float3x3 randRotMatrix = angleAxis3x3(rand(positionOS) * UNITY_TWO_PI, float3(0, 0, 1.0f));

				// Rotate around the bottom of the blade a random amount.
				float3x3 randBendMatrix = angleAxis3x3(rand(positionOS.zzx) * _BendDelta * UNITY_PI * 0.5f, float3(-1.0f, 0, 0));

				// Tip of grass needs to have more bending than the base.
				...
				float minWidth = _BladeWidthMin;
				float minHeight = _BladeHeightMin;

				float width  = lerp(_BladeWidthMin, _BladeWidthMax, rand(positionOS.xzy));
				float height = lerp(_BladeHeightMin, _BladeHeightMax, rand(positionOS.zyx));
				float forward = rand(positionOS.yyz) * _BladeBendDistance;
				...
				for (int i = 0; i < BLADE_SEGMENTS; ++i)
				{
					...
					triStream.Append(TransformGeomToClip(positionOS, float3( offset.x, offset.y, offset.z), transformationMatrix, float2(0, t)));
					triStream.Append(TransformGeomToClip(positionOS, float3(-offset.x, offset.y, offset.z), transformationMatrix, float2(1, t)));
				}

				// Add the final vertex at the tip of the grass blade.
				triStream.Append(TransformGeomToClip(positionOS, float3(0, forward, height), tipTransformationMatrix, float2(0.5, 1)));
		...
		*/
        ...
        Pass
        {
            ...
            float4 frag (GeomOutput i) : SV_Target
            {
				float4 color = tex2D(_BladeTexture, i.uv) * _TintColor;
