...
SHIPPED:
1. O(N^2) TURN END (the big one). EvaluateDynamicTransforms ran in the finally of EVERY RunForTrigger, and ApplyDynamicTransforms itself rescans every card in every pile calling GetActiveDynamicTransformEffect (a full runtime effect build) per card. A 20-card deck at turn end was ~400 effect-list rebuilds. Now gated by HasAnyDynamicIdentityRows(player, combatState): a per-Player presence cache keyed on CardEditorRuntimeCacheVersion + a stamp of all four combat piles (card identity AND upgrade level, since upgrades mutate in place without a version bump). Mirrors the existing QuestPresenceEntry pattern. Never skipped while state.Entries.Count > 0.
   *** THE BREAK THE REVIEW CAUGHT: the presence scan used GetEffectsForDescription, but the path it gates uses GetRuntimeEffectsIncludingBorrowedSources. For a CREATED card with legacy effect sources, the runtime list also injects the SOURCE card's rows - so a DynamicIdentity row borrowed from another card was INVISIBLE to the gate. That card would have silently stopped transforming forever. Fixed by conservatively returning true for any created card with HasEffectSourceCard. False positives only cost the scan; false negatives delete a feature.
   Verified safe for: temp grants/auras (CombatHasAnyGrantedEffects short-circuits to true and covers BOTH the temporary controller and the aura controller), cards drawn/created mid-combat (pile stamp), upgrade level changes (in the stamp), replacement cards (state.Entries bypass), co-op (cache is per Player).
2. MID-COMBAT DISK WRITE. Quest completion called FlushIfDirty on the counter and power stores in a finally - a synchronous File.WriteAllText mid-combat, on a OneDrive path where writes stall 100ms+. Now flushes immediately when NOT in combat (durability unchanged there) and defers to the existing turn-end/combat-end seams during a fight. First attempt removed the flush entirely; the review flagged the durability regression (no run-end seam exists), hence the in-combat-only deferral.
3. COMPILED REGEX PER DESCRIPTION BUILD. GetCache(card, target, isUpgradePreview) merged custom keywords and called BuildRegex -> new Regex(RegexOptions.Compiled), then DISCARDED the Cache (only the no-arg cache was stored). Compiled regex emits IL. Now memoised per card via ConditionalWeakTable, keyed on runtime cache version + base signature + isUpgradePreview + the ordered custom keyword names/descriptions, so renaming/adding/removing a keyword rebuilds it. Reviewed SAFE.
Build 0 errors / 278 warnings. Deployed, distribution refreshed.
METHOD NOTE worth keeping: the pattern that works on this codebase is implement -> hostile review specifically for FEATURE LOSS -> fix -> ship. Both times a reviewer was pointed at my own work it found something real (the green fix, and now the borrowed-source gate).
Next Step: play a combat. Expect fewer stutters at turn boundaries especially. If any custom card with borrowed effect sources stops transforming, the gate is still wrong - revert item 1 first.
## 2026-07-27 (4) - Two confirmed combat performance fixes: mid-combat disk write + per-description Regex compile

Hypothesis: (1) FlushIfDirty in quest completion finally fires a synchronous File.WriteAllText mid-combat; (2) GetCache(card,target,isUpgradePreview) compiles a new Regex(RegexOptions.Compiled) on every description build.
Finding: Both confirmed and fixed.
Evidence:
- CardEditorQuestEffects.cs:439-440 called both FlushIfDirty() methods inside a finally block triggered when a quest counter crosses its threshold mid-combat. Both store classes already had deferFlush logic and existing flush seams at turn-end (CardEditorMod.cs:3312-3313) and combat-end (3377-3378), so the mid-combat flush was redundant.
- CardEditorVanillaKeywordSupport.cs GetCache(card,target,isUpgradePreview) called BuildRegex(mergedTerms.Keys) which constructs new Regex(..., RegexOptions.Compiled) and returned the result without storing it. Every description build for any card with custom keywords paid the IL-emit cost on every call.
Fix 1 (CardEditorQuestEffects.cs): removed the two FlushIfDirty() calls from the finally block. In-memory state (_cache + _dirty flag) is updated immediately; disk write deferred to next turn-end or combat-end seam. Durability window: if process is killed between quest completion and the next seam, the quest-completion increments/grants are in memory but not yet on disk. Maximum window: one player turn (or end of combat, whichever fires first). No system reads the file between quest completion and the seam (file is only read at Load(), which runs at run-start, not mid-combat).
Fix 2 (CardEditorVanillaKeywordSupport.cs): added ConditionalWeakTable<CardModel, PerCardKeywordCacheEntry> _perCardCache. GetCache(card,...) builds a composite signature from version + baseSignature + isUpgradePreview + summary names+descriptions, checks the per-card memo, and only calls BuildRegex when the signature differs. Cards can be GC'd freely (ConditionalWeakTable). Staleness analysis: (1) custom keyword renamed/added/removed mid-session: CardEditorDefinitionStore.UpsertKeyword/ReplaceDefinitions bumps CardEditorRuntimeCacheVersion, which changes the version component of the signature, triggering a rebuild; (2) upgrade preview vs normal render: isUpgradePreview is part of the key, so the two render paths get independent cache entries; (3) two different cards with different keywords: ConditionalWeakTable keys on card object identity, so each card has its own entry; (4) card with no custom keywords: GetCustomKeywordSummaries returns empty, GetCache(card,...) returns baseCache immediately (before reaching the memo path) - no per-card entry is stored.
Build: 0 errors, 278 warnings (at baseline).
Next Step: play a combat with a card that has custom keywords and confirm no visible tooltip/highlighting regressions.
